    ...
    <Tabs defaultValue="scheduling" className="w-full">
      <TabsList className="grid w-full grid-cols-2 md:grid-cols-5 h-auto bg-light-grey p-2 rounded-xl shadow-inner"> {/* Refined TabsList styling */}
        {features.map((feature) => (
          <TabsTrigger key={feature.value} value={feature.value} className="flex flex-col items-center gap-2 p-4 h-full data-[state=active]:bg-background data-[state=active]:shadow-subtle data-[state=active]:text-basil data-[state=active]:font-semibold rounded-lg transition-all duration-200 text-stone hover:text-charcoal font-averta"> {/* Refined TabsTrigger styling */}
            ...
            <CardContent className="p-0">
              <div className="grid md:grid-cols-2 items-center bg-background"> {/* Explicit background */}
                <div className="p-8 lg:p-12 space-y-6 order-2 md:order-1"> {/* Increased padding and spacing */}
                  ...
                  <Button variant="custom-primary" size="custom-sm" className="shadow-md hover:shadow-lg transition-all duration-300">Learn More</Button> {/* Button with shadow */}
                </div>
